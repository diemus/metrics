...
func InitPushProcessMetrics(pushURL string, interval time.Duration, username, password string, extraLabels string) error {
	writeMetrics := func(w io.Writer) {
		WriteProcessMetrics(w)
	}
	return InitPushExt(pushURL, interval, username, password, extraLabels, writeMetrics)
}
...
// in this case metrics are pushed to all the provided pushURL urls.
func InitPush(pushURL string, interval time.Duration, username, password string, extraLabels string, pushProcessMetrics bool) error {
	writeMetrics := func(w io.Writer) {
...
// It is OK calling InitPush multiple times with different pushURL -
// in this case metrics are pushed to all the provided pushURL urls.
func (s *Set) InitPush(pushURL string, interval time.Duration, username, password string, extraLabels string) error {
	writeMetrics := func(w io.Writer) {
		s.WritePrometheus(w)
	}
	return InitPushExt(pushURL, interval, username, password, extraLabels, writeMetrics)
...
func InitPushExt(pushURL string, interval time.Duration, username, password string, extraLabels string, writeMetrics func(w io.Writer)) error {
	if interval <= 0 {
